...
- This repository uses TypeScript for automation scripts in `scripts/` and `.github/scripts/`
- Scripts are executed with `tsx` (not plain `node`) so `.ts` files run directly
- Run all local checks with `pnpm checks`
- Run TypeScript type checks with `pnpm typecheck`
